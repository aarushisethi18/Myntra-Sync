...
/** Reads the persisted context snapshot without collecting browser signals again. */
export async function getLiveContext(session: Session): Promise<LiveContext> {
  const response = await fetch(`${API_BASE_URL}/context/live`, { headers: { Authorization: `Bearer ${session.access_token}` } });
