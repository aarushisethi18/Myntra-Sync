...
export default function CatalogPage() {
  const { session } = useAuth(); const navigate = useNavigate(); const { collection = "all" } = useParams();
  ...
  const wish = useCallback(async (product: Product) => { if (!session) return; const exists = wished.has(product.id); if (exists) await removeFromWishlist(session, product.id); else await addToWishlist(session, product.id); setWished((current) => { const next = new Set(current); exists ? next.delete(product.id) : next.add(product.id); return next; }); }, [session, wished]);
  ...
  return <div className="min-h-screen bg-white text-[#282C3F]"><ShopHeader onSearch={(value) => setSearchInput(value)} onCategory={(value) => setCategory(value)} />
    <main className="max-w-[1440px] mx-auto px-6 md:px-12 py-9"><div className="flex flex-wrap items-end justify-between gap-4 mb-7"><div><p className="text-[10px] font-extrabold tracking-[.2em] text-[#FF3F6C] uppercase">Discover more</p><h1 className="font-editorial text-3xl mt-1">{titles[collection] ?? titles.all}</h1><p className="text-sm text-[#94969F] mt-1">{total} matching products</p></div><select value={sort} onChange={(e) => setSort(e.target.value)} className="border border-[#EAEAEC] rounded-lg px-3 py-2 text-sm"><option value="">Sort by</option><option value="price_asc">Price: low to high</option><option value="price_desc">Price: high to low</option><option value="rating_desc">Rating: high to low</option></select></div>
      ...
      {loading ? <div className="py-24 text-center text-[#94969F]">Loading products…</div> : products.length === 0 ? <div className="py-24 text-center text-[#94969F]">No products match these filters.</div> : <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">{products.map((product) => <ProductCard key={product.id} product={product} wished={wished.has(product.id)} onOpen={() => setSelected(product)} onWish={() => void wish(product)} onBrandOpen={() => setSearchInput(product.brand)} />)}</div>}
    ...
    </main>{selected && <ProductDetails product={selected} onClose={() => setSelected(null)} onCart={async () => { if (session) { await addToBag(session, selected.id, selected.sizes[0] || "One Size"); setSelected(null); } }} onPurchase={() => navigate("/bag")} onDwell={() => {}} />}</div>;
}
