...
export default function CatalogPage() {
  ...
  const wish = useCallback(async (product: Product) => { if (!session) return; const exists = wished.has(product.id); if (exists) { await removeFromWishlist(session, product.id); trackAnalytics({ eventType: "WISHLIST_REMOVE", productId: product.id, brand: product.brand, category: product.category }); } else { await addToWishlist(session, product.id); trackAnalytics({ eventType: "WISHLIST_ADD", productId: product.id, brand: product.brand, category: product.category }); } setWished((current) => { const next = new Set(current); exists ? next.delete(product.id) : next.add(product.id); return next; }); }, [session, wished, trackAnalytics]);
